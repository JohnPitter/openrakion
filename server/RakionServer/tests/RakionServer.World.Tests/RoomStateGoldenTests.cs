...
namespace RakionServer.World.Tests
{
    ...
    public class RoomStateGoldenTests
    {
        ...
        [Fact]
        public void PlayerRecord_EndsWithEquipTail_ForObserverFix()
        {
            // O bloco de equip default (observer-fix) fica nos ÚLTIMOS 11B do record (record variável: nome até \0
            // + resto). Sem ele o 2º humano vira "observador" no stage. Vale p/ qualquer nome.
            byte[] tail = { 0x11, 0x00, 0x01, 0x01, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00 };
            Assert.Equal(tail, WorldServer.BuildPlayerRecord("Heroi2", 0, 1)[^11..]);
            Assert.Equal(tail, WorldServer.BuildPlayerRecord("JP", 0, 1)[^11..]);
        }
    }
}
