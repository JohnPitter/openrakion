...
namespace RakionServer.World.Tests
{
    ...
    public class BotLifecycleTests
    {
        ...
        [Fact]
        public void ResetMatch_ClearsObservedHumanHull()
        {
            var f = NewField();
            f.ExpandHumanHull(10f, -5f);               // chão "pisado" do mapa anterior
            f.ExpandHumanHull(40f, 20f);
            Assert.True(f.HasHumanHull);

            f.ResetMatch();

            // Rematch (possivelmente noutro mapa): o hull velho deixaria o bot "pisar" fora dos limites do
            // mapa novo. Tem de voltar ao estado "ainda não observado".
            Assert.False(f.HasHumanHull);
            Assert.Null(f.HumanHull(2f));
        }
