...
namespace RakionServer.World.Network
{
    ...
    public static class BotMovement
    {
        ...
        /// <summary>Âncora reliable que declara um membro Player vivo no conjunto de combate.</summary>
        public const ushort MsgAnchorState = 0x830c;
        ...
        /// <summary>Declara no canal reliable que o alvo é uma entidade Player viva. O bot usa a mesma
        /// sequência monotônica dos seus datagramas, como um remetente humano.</summary>
        public static byte[] BuildCombatAnchorDatagram(BotPlayer bot, int seat, int targetSeat)
        {
            using var w = new PacketWriter();
            w.WriteWord(MsgAnchorState);
            w.WriteUInt32(bot.UdpSeq++);
            w.WriteByte((byte)seat);
            w.WriteByte((byte)seat);
            w.WriteByte(0x01);
            w.WriteWord((ushort)targetSeat);
            w.WriteWord(0x002a);
            w.WriteWord(0x0191);
            w.WriteUInt32(4);
            w.WriteUInt32(1);
