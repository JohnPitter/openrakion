...
## Power User não requer mais patch de mensagem

O callback real de sucesso `0x34` foi reconstruído em 2026-07-15. O World local agora envia
`[status=0][gold][cash][powertime][powerlevelpoint][presentCount][presentIds...]`, exatamente como
`FUN_004281B0` adapta a operação interna `0x17` para `FUN_00474F50` no cliente. Remova qualquer
substituição antiga da mensagem de erro de status `2`; ela não participa mais do fluxo de sucesso.
