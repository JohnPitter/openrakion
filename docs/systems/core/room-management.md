...
`mode=0` apenas ecoa `0x25` nessa etapa; não ocupa o array de fields do World original.
...
| `0x3B` | `Op_FieldCreate` → `ClientSession.Rooms` | `mode=0` ecoa `0x25` e prepara field interno não pesquisável para o lifecycle solo; modos `1..4` publicam `Field` completo |
...
| criar sala solo | compatível e não pesquisável | `mode=0` responde `0x25`; o .NET prepara field interno para `0x43`, mas ele nunca entra na lista pública |
