# Rakion v258 — protocolo World legado (reconstruído do `worldserv.exe`)

> **Documento histórico de transporte e capturas.** O registro canônico de requests,
> estados, rotas efetivas e conflitos do servidor atual está em
> [`../protocol/world.md`](../protocol/world.md). A seção antiga de
> handlers abaixo foi superada pela auditoria canônica e não deve orientar novos nomes.
...
No case `0x0C`, `this+0x5b18 == 0` chama `FUN_0041f6c0`; quando já autenticado chama
`FUN_0042a310`. A inspeção posterior do assembly mostrou que `FUN_0042a310` também é um
caminho de login/relogin, não um dispatcher in-game geral. O detalhamento atualizado e a
lista exata de lacunas da jump table estão em `../protocol/world.md`.
