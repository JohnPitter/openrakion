...
| Contador HIT×N sobre o bot | **em aberto — mecanismo existe, elo final não cravado** | reteste em 29/07/2026: com reação visível funcionando, o número flutuante segue ausente. **Não é teto arquitetural** — o gate `[vítima+0x394]` é contornado pelo `RakionClientPatch` (cave em `0x351533e9`), que chama `AddHitCount` direto no jogador **local**, onde o contador vive. Cadeia verificada estaticamente ponta a ponta: `ConfirmHit` devolve sequência crescente → `TakeDamage` grava `LastAttackerSeat`/`LastAttackerHitSequence` → `PublishBotLifecycles` escreve os 8 campos → a DLL lê o arquivo (provado: aplicou `reacao de dano` e `lifecycle` no log da partida). Binários deployados batem hash com o build desta branch. Falta cravar por que o ramo do HIT local não dispara — ele exige `localPlayer() == player`, caminho distinto do da reação, que age sobre a entidade do bot |
