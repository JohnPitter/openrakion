...
| Contador HIT×N sobre o bot | **correção implementada; reteste visual pendente** | a auditoria encontrou uma indireção incorreta: o cave original executa `mov eax,[0x352b3630]; call eax`, portanto `0x352b3630` contém o ponteiro da função que retorna o jogador local. A DLL chamava o endereço da variável como código; a exceção era absorvida pelo `__except` e o contador nunca era aplicado. `RakionClientPatch` agora desreferencia o ponteiro antes de testar `localPlayer() == player`. Build x86 passa com `/W4 /WX`; falta confirmar o número flutuante no cliente |
