...
| Contador HIT×N sobre o bot | **segunda correção implementada; reteste visual pendente** | após corrigir a indireção de `0x352b3630`, o trace provou `attackerSeat=0/hitSequence=6`, lifecycle aplicado no bot `seat=10` e nenhuma chamada `HIT local`: o hook só visitava a entidade remota e nunca satisfazia `localPlayer() == player`. A ponte agora usa o update remoto como dispatcher da game thread, resolve o jogador local e aplica nele a sequência confirmada. Build x86 passa com `/W4 /WX`; falta confirmar o número flutuante no cliente |
