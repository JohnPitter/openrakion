...
| Bot no fio | `AddBotButtonCommandE2ETests`, `BotMovementE2ETests` e `BotStageValidationTests` | O `0x47` exato do botão adiciona o bot sem disconnect; movimento sintético recebido; perseguição converge; dois humanos recebem o bot; `kind=Attack` não é confundido com contato e não reduz HP, inclusive em tunneling |
