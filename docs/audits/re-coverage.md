...
| 10 | [`systems/gameplay/cells-creatures-npc.md`](../systems/gameplay/cells-creatures-npc.md), [`npc-stat-curves.md`](../systems/gameplay/npc-stat-curves.md) e [`npc-family-nak.md`](../systems/gameplay/npc-family-nak.md) | RE estático e compatibilidade headless completos; dinâmica visual pendente | golden source: 47 tipos/99 níveis, 43 classes carregáveis; `NpcBlackDragon*` ausente. As 24 séries de `creatures.dat` fecham os 8.118 bytes, dez labels do painel estão ligados aos offsets e Attack é a base de dano; Armor é zero e não existe curva AP separada. CP/custo, três slots/estados, ausência de cooldown, liberação e refund de 30% estão provados. Scan integral de 22 refs e das 130 entradas Lua fecha CP inicial, init blob e ausência de regeneração passiva. `IsEnemy`, master, faixas red/blue/gray, friendly fire, validação de dano e `CNpcWatcher` fecham a política comum de targeting. Nak fecha as quatro variantes, 29 eventos locais, reação, `Shoot_Poison`, perseguição, morte, faixa `3.0f` e campos próprios. As três famílias de init e 269/269 eventos estão catalogadas. O único caller de late join e sua ordem/guarda estão fechados; `0x8307/08/09/0B/0C/10/12` possuem parsers. Faltam goldens runtime, efeitos exatos do poison, demais famílias e validação gráfica de ataques/late join; autoridade server-side é opcional |
