...
| 26 | [`protocol/client-integrity.md`](../protocol/client-integrity.md) | RE completo desta build; proteção moderna headless | GameGuard separado; login/ChCode, MD5 global, modos, `BB/BC`, feature flag e probes fechados. Imports, vtable e consumidores de `_pRakionWorldNet` provam ChCode sem call site e packet-speed como no-op dormente. Update assinado e ticket vinculado a app/build implementam o gate de rollout; a suíte World atual possui 817 testes verdes e o launcher mantém 11. A DLL centraliza patches e IP com gate da build golden; validação visual segue pendente |
