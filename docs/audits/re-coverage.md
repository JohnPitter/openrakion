...
| 10 | [`systems/gameplay/cells-creatures-npc.md`](../systems/gameplay/cells-creatures-npc.md), [`npc-stat-curves.md`](../systems/gameplay/npc-stat-curves.md), [`npc-family-nak.md`](../systems/gameplay/npc-family-nak.md) e [`npc-family-panzer.md`](../systems/gameplay/npc-family-panzer.md) | RE estático e compatibilidade headless completos; dinâmica visual pendente | golden source: 47 tipos/99 níveis, 43 classes carregáveis; `NpcBlackDragon*` ausente. As 24 séries de `creatures.dat` fecham os 8.118 bytes e Attack é a base de dano. CP/custo, slots, ausência de cooldown, liberação e refund de 30% estão provados. `IsEnemy`, master, times, friendly fire, dano e `CNpcWatcher` fecham a política comum. Nak fecha 29 eventos, `Shoot_Poison`, perseguição e morte. Panzer fecha 34 eventos, `Attack_01/02`, stab/lash, FireOrHit e seleção `0.6/0.9`. As três famílias de init, 269 eventos, late join e parsers `0x8307/08/09/0B/0C/10/12` estão fechados. Faltam goldens runtime, efeitos/hitboxes exatos, famílias após Panzer e validação gráfica; autoridade server-side é opcional |
