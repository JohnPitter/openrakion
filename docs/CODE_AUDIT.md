...
- **`ClientSession.cs`** → extrair `OracleReplay`/`LobbyEntryReplay` (os `_rNN`, `TryHandleLobbyEntry`, `SendLoginResponseReplay`, `ApplyEquipAppearance`) e `InventoryProtocol`/`ShopSession`; agrupar o god-object de estado em `CharProgress`/`FieldState`/`Wallet`.
