# hitcount.ps1 — compila e roda o hook externo de fn-de-hit 0x350d69e0 (diagnóstico do HIT×N com o bot).
# NÃO injeta DLL (só WriteProcessMemory, que o anti-tamper permite). Rode com o jogo JÁ no stage.
#
# Uso:  .\hitcount.ps1                 # 2 humanos SÓ (baseline: bata e veja se fn-de-hit 0x350d69e0 é chamado)
...
Write-Host "Compilando capture_hitgate.exe (x86)..." -ForegroundColor Cyan
$cmd = "call `"$Vcvars`" x86 && cd /d `"$here`" && cl /nologo /O2 /EHsc capture_hitgate.cpp /Fe:$exe /link advapi32.lib"
