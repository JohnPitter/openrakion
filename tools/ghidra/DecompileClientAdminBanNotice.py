# -*- coding: utf-8 -*-
# Extrai builders e consumidores de SendAdminBan/SendAdminNotice.
# Saida: C:\temp\client_admin_ban_notice.txt
# @category Rakion
...
for symbol in currentProgram.getSymbolTable().getAllSymbols(True):
    ...
    if "sendadminban" not in name and "sendadminnotice" not in name:
        continue
...
with open(r"C:\temp\client_admin_ban_notice.txt", "w") as output:
...
print("Client AdminBan/AdminNotice: %d simbolos, %d funcoes" % (len(symbols), len(functions)))
