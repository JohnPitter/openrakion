# -*- coding: utf-8 -*-
# Localiza o export/import SendGMOperation e decompila consumidores imediatos.
# Saida: C:\temp\client_gm_operation.txt
# @category Rakion
...
for symbol in currentProgram.getSymbolTable().getAllSymbols(True):
    if "gmoperation" not in symbol.getName(True).lower():
        continue
...
with open(r"C:\temp\client_gm_operation.txt", "w") as output:
...
print("Client GMOperation: %d simbolos, %d funcoes" % (len(symbols), len(functions)))
